...
**Known limitation.** `ProtoObject::clone()` does not carry the source's own attributes, so the returned object is **empty**: `protoCore.ImmutableObject({a: 1}).a` is `undefined`. The QuickJS-side module appeared to copy the properties only because it converted the argument JavaScript → protoCore → JavaScript through `TypeBridge`, which rebuilt the object along the way; the native binding has no such round trip. Copying the attributes inside protoJS is not possible through protoCore's public API either: `getOwnAttributes()` returns a sparse list keyed by attribute hash, which does not yield the attribute names needed to rewrite them. Making these helpers copy their argument therefore needs a change in protoCore (a clone that carries attributes, or public attribute-name enumeration).
