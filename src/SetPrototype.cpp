                       ...
                       const proto::ProtoObject* objectProto)
{
    ...
    auto installMethod = [&](const char* name, proto::ProtoMethod fn) {
        ...
        if (mObj) setProto = setProto->setAttribute(ctx, ks, mObj);
    };

    installMethod("add",     setAdd);
    installMethod("has",     setHas);
    installMethod("delete",  setDeleteFn);
    installMethod("clear",   setClear);
    installMethod("forEach", setForEach);
    installMethod("values",  setValues);
    installMethod("keys",    setKeys);
    installMethod("entries", setEntries);
